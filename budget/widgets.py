...
class GroupedSelect(django.forms.Select): 
...
class GroupedChoiceField(django.forms.ChoiceField):
    def __init__(self, choices=(), required=True, widget=GroupedSelect, label=None, initial=None, help_text=None):
        super(django.forms.ChoiceField, self).__init__(required, widget, label, initial, help_text)
    ...
    def clean(self, value):
        ...
        """
        value = super(django.forms.ChoiceField, self).clean(value)
        if value in (None, ''):
            value = u''
        value = django.forms.util.smart_unicode(value)
        if value == u'':
            return value
        valid_values = []
        for group_label, group in self.choices:
